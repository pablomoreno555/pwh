...
from utils import navigation, object_finding, object_placement, object_scanning
...
def take_photos():
    """
    Take photos of the scene from the robot's perspective, with different camera angles. Right before taking each picture, we change the resolution of the rendered image frame to Quad HD for better definition. Right after taking each picture, we reset to the initial resolution to make the simulation faster
    """
    ...
    # event = controller.step(action='ChangeResolution', x=2560, y=1440)
    image_data = event.frame
    # controller.step(action='ChangeResolution', x=1200, y=800)
...
def trigger_vqa():
    ...
    prompt3 = f"You are an excellent problem solver. Given the problem identified above, the list of actions that the robot can perform, and the objects available in the scene, you can determine if there is a clear alternative way for the robot to accomplish the given instruction despite the identified problem. Your reply must be one of two types:\nType A: If there is a clear alternative way to achieve the instruction by using only the actions and objects previously defined, reply with a new plan composed of the steps needed to accomplish it. For example, if there is a clear alternative plan to accomplish the instruction 'light up the room' by turning on the lamp, your reply should look like:\nA\n1. find: {{\"object\": \"Lamp\"}}\n2. go_to: {{\"object\": \"Lamp\"}}\n3. toggle: {{\"object\": \"Lamp\"}}\nAgain, your output must be limited to a list of action names with arguments, without explanations, introductory paragraph, or concluding statement, and with every step in a new line.\nType B: If the robot is unable to accomplish the given instruction by using only the actions and objects previously defined, you should reply with the specific problem that the robot is facing. Your reply should look like:\nB\nThe robot was not able to accomplish the instruction. It failed at step '{step_failed}' because/due to...\nYou should give a Type A reply only when there is a clear alternative way to achieve the instruction given by the user. For example, if they ask for something to eat, some alternatives may include an apple or bread, or if they request something to read, some options could be a book or a newspaper. However, if, for example, the robot is not able to put the specified object in the specified location, do not give a Type A reply to put the object somewhere else, just give a Type B reply. Also note that the robot is not able to move furniture or considerably large items. The first line of your reply must be the character 'A' or 'B', respectively. Now, please do this for the case described above, in which the instruction is: '{instruction}'."
